...
function generate_breadcrumbs() {
    ...
    foreach ($breadcrumbs as $index => $crumb) {
        ...
        if ($is_last) {
            ...
            echo '<span class="ml-1 text-xl font-medium text-blue-700 md:ml-2">> ' . htmlspecialchars($crumb['label']) . '</span>';
            echo '</li>';
        } else {
            echo '<li class="inline-flex items-center">';
            echo '<a href="' . htmlspecialchars($crumb['url']) . '" class="inline-flex items-center text-xl font-medium text-gray-700 hover:text-gray-900">';
            if ($index > 0) {
